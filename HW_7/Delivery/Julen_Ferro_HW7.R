...
printing <- function(string, array){
  print(paste0(string," ", "Class 1: ", format(array[1], nsmall = 2), " Class 2: ", format(array[2], nsmall = 2)))
  print(paste0("            " , "Class 3: ", format(array[3], nsmall = 2), " Class 4: ", format(array[4], nsmall = 2)))
}
...
run <- function(neurons, df_train, df_test, bool){
  ...
  results <- NN_2 %>% evaluate(as.matrix(X_test), y_test.ohe)
...
# Check the pattern of the predictions with one neuron
  ...
  return(c(round(results[2], 2), round(results[1], 2)))
}
...
print(paste0("For one neuron in the hidden layer, loss: ", format(loss, nsmall = 2), " Accuracy: ", format(accuracy, nsmall = 2)))
...
for(x in list){
  print(paste0("With ", x ," neurons: In this model, Accuracy is: ", format(accuracy[x], nsmall = 2)))
}
...
print(paste0("In this model, loss: ", format(loss, nsmall = 2), " Accuracy is: ", format(accuracy[i], nsmall = 2)))
